...
import { createWebSocketService, getWebSocketUrl, type WebSocketService } from '@/services/websocket';
import { updateMessageSyncStatus } from '@/services/db';
import {
  setWebSocketStatus,
  setWebSocketError,
  setConnectedAt,
...
import { selectJWTToken } from '../slices/deviceSlice';
import { receiveMessageAction } from './messageSaga';
...
function* handleConnectWebSocket(): Generator<unknown, void, unknown> {
  try {
    ...
    log.error('Failed to connect WebSocket', error, {
      errorMessage,
      ...errorDetails,
      reconnectAttempts: (yield select((state: any) => state.websocket.reconnectAttempts) as unknown) as number,
    });
 ...
 */
function* handleMessageSentConfirmation(payload: any): Generator<unknown, void, unknown> {
  try {
 ...
 * Convert WebSocket message format to domain Message format
 */
function convertWebSocketMessageToDomain(wsMessage: WebSocketMessage): any {
...
function* watchWebSocketReconnect(): Generator<unknown, void, unknown> {
  ...
  while (true) {
    yield take((action: any) => {
      return (
        action.type === 'websocket/setWebSocketStatus' &&
        (action.payload === 'disconnected' || action.payload === 'error')
      );
    });
...
function* handleWebSocketError(action: { type: string; payload: string | null }): Generator<unknown, void, unknown> {
  ...
  if (error) {
    ...
    const status = (yield select((state: any) => state.websocket.status) as unknown) as string;
