import { call, put, takeEvery, takeLatest, take, select, fork, delay, cancelled, all, cancel } from 'redux-saga/effects';
...
export const pullGroupsReplicationAction = (location?: { latitude: number; longitude: number; radius: number }) => ({
...
function* handlePullGroupsReplication(action: { type: string; payload?: { latitude: number; longitude: number; radius: number } }) {
  ...
  try {
    log.debug('Pulling groups from replication', { hasLocation: !!action.payload });
...
function* handleUpdateGroupName(action: { type: string; payload: { groupId: string; name: string } }): Generator<unknown, void, unknown> {
  ...
  try {
    ...
    const updatedGroup = (yield call(updateGroupNameService, groupId, name)) as unknown as Group;
 ...
 */
async function fetchGroupDetailsHelper(groupId: string, deviceId: string | null) {
  try {
 ...
 */
function* handleFetchGroupsDetails(
  ...
  const groupsToFetch = groupIds.filter((groupId) => !fetchingGroupsDetails.has(groupId));
  ...
  try {
    const device = (yield select(selectDevice)) as unknown as { id?: string } | null;
    ...
    log.debug('Group details collected and stored in Redux', { count: details.length });
  } catch (error) {
    log.error('Failed to fetch groups details', error, { groupIds: groupsToFetch });
  } finally {
