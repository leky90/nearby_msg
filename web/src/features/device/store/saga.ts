...
import {
  getToken,
  getDeviceId,
} from "@/features/device/services/device-storage";
...
function* handleFetchDevicesByIds(action: {
  type: string;
  payload: string[];
  ...
  try {
    ...
    for (const deviceId of deviceIds) {
      try {
        const deviceDoc = (yield call(async () => {
          return await db.devices.findOne(deviceId).exec();
        })) as unknown;
        if (
          deviceDoc &&
          typeof deviceDoc === "object" &&
          deviceDoc !== null &&
          "toJSON" in deviceDoc
        ) {
          const device = (
            deviceDoc as { toJSON: () => unknown }
          ).toJSON() as Device;
          devices.push(device);
        }
      } catch {
    ...
    if (devices.length > 0) {
      ...
      log.debug("Devices fetched and updated in Redux", {
  ...
  try {
    ...
    if (device) {
      ...
      if (token) {
        ...
        log.debug(
          "Device not in RxDB but token exists, triggering pull replication"
        );
    ...
    yield put(
      setDeviceError("Không thể tải thông tin thiết bị. Vui lòng thử lại sau.")
    );
    yield put(setDevice(null));
  } finally {
...
function* handleUpdateDevice(action: {
  type: string;
...
function* handleClearDevice() {
  ...
  if (isClearingDevice) {
    log.warn(
      "[CLEAR_DEVICE] Clear operation already in progress, skipping duplicate call"
    );
    return;
  }
  ...
  try {
    ...
    log.info("[CLEAR_DEVICE] Step 2: Retrieved device ID", {
      deviceId,
    ...
    if (deviceId) {
      try {
        log.info(
          "[CLEAR_DEVICE] Step 3: Calling API to delete device on server",
          { deviceId }
        );
        ...
        log.info(
          "[CLEAR_DEVICE] Step 3: Device deleted on server successfully",
          { deviceId }
        );
      } catch (apiError) {
        ...
        log.warn(
          "[CLEAR_DEVICE] Step 3: Failed to delete device on server (continuing with local clear)",
          apiError,
          { deviceId }
        );
      }
      ...
      log.info(
        "[CLEAR_DEVICE] Step 3: Skipping server deletion (no device ID)"
      );
    }
    ...
    yield put(setDeviceLoading(false));
    log.info(
      "[CLEAR_DEVICE] Step 7: DeviceLoading set to false - App.tsx will show onboarding automatically"
    );

    log.info(
      "[CLEAR_DEVICE] Step 8: All user data cleared, onboarding required - PROCESS COMPLETE"
    );
  } catch (error) {
    ...
    log.info(
      "[CLEAR_DEVICE] Clear operation completed, guard remains set to prevent re-triggers"
    );
  }
}
...
function* watchDevicesRxDBChanges() {
  try {
    const channel: EventChannel<Device[]> = yield call(
      createDevicesEventChannel
    );
    ...
    try {
      while (true) {
        ...
        yield put(setDevicesById(devices));
        log.debug("Devices RxDB listener: Updated Redux store", {
          count: devices.length,
        });
      }
    } finally {
  ...
  return eventChannel<Device[]>((emit) => {
    ...
    getDatabase()
      .then((db) => {
        ...
        const subscription = db.devices.$.subscribe(
          async (changeEvent: any) => {
            if (!isActive) return;
        ...
        db.devices
          ...
          .then((docs) => {
            ...
            emit(devices);
            log.debug("Devices RxDB initial state emitted", {
              count: devices.length,
            });
          })
          .catch((err) => {
