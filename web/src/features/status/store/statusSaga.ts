...
function* watchUserStatusRxDBChanges() {
  try {
    const channel: EventChannel<UserStatus | null> = yield call(
      createUserStatusEventChannel
    );
    ...
    try {
      while (true) {
        ...
        yield put(setUserStatus(status));
        log.debug("User status RxDB listener: Updated Redux store", {
          hasStatus: !!status,
        });
      }
    } finally {
  ...
  return eventChannel<UserStatus | null>((emit) => {
    ...
    getDatabase()
      .then((db) => {
        ...
        const subscription = db.user_status.$.subscribe(
          async (changeEvent: any) => {
            ...
            try {
              ...
              const status =
                docs.length > 0 ? (docs[0].toJSON() as UserStatus) : null;
              emit(status);
              ...
              log.error(
                "Failed to query user status after collection change",
                err
              );
            }
          }
        ...
        db.user_status
          ...
          .then((docs) => {
            ...
            const status =
              docs.length > 0 ? (docs[0].toJSON() as UserStatus) : null;
            emit(status);
            log.debug("User status RxDB initial state emitted", {
              hasStatus: status !== null,
            });
          })
          .catch((err) => {
